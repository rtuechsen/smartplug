...
export function App(): JSX.Element {
	...
	const [remainingSessionTimeSeconds, setRemainingSessionTimeSeconds] =
	...
	async function displayError(
		message: string,
		returnToLoginPage: boolean = false,
	): Promise<void> {
		if (isErrorOpen) {
	...
	async function getRemainingSessionTime(): Promise<void> {
		...
		if (!response.ok) {
			...
			const sessionExpiryDate: Date = new Date(
				responseData.session_expiry_date,
			);
			const remainingTimeSeconds =
				(sessionExpiryDate.getTime() - Date.now()) / 1000;
