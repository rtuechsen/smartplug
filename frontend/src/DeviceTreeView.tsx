...
/**
 * An interface copied from the Material UI library needed make the type known to the compiler.
 */
export interface RichTreeViewItemSlotOwnerState {
...
export function DeviceTreeView({
	displayError,
}: DisplayErrorCallbackProps): JSX.Element {
	const ref = React.useRef<HTMLDivElement>(null);

	// state for holding the data of the tree, triggers updates to the tree if the data changes
	const [deviceTreeDataState, setDeviceTreeDataState] =
		React.useState<DeviceTreeItemData[]>();
	...
	const [deviceTreeDataArrivedState, setDeviceTreeDataArrivedState] =
	...
	return (
		...
		<Box ref={ref} sx={{ minHeight: "max-content", width: "fit-content" }}>
			...
			{deviceTreeDataState == undefined ||
			expandedIdsState.length === 0 ? null : (
				<RichTreeView
					items={deviceTreeDataState} // the data for the tree, updates when the state changes
					slots={{
						item: DeviceTreeItem as React.JSXElementConstructor<TreeItem2Props>,
					}} // a custom tree item that includes label, icons and buttons
					...
					slotProps={{
						...
						} as SlotComponentPropsFromProps<
							TreeItem2Props,
							object,
							RichTreeViewItemSlotOwnerState
						>,
					}} // need to cast our properties to the type the slot is expecting
					...
					} // because we set the expanded items explicitly, user interaction will not work anymore => need to apply user interactions manually
				/>
			)}
		</Box>
	);
